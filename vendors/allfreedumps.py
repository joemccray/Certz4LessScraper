...
async def discover_allfreedumps_tasks(
    ...
    res = await fetch_with_retry(
        client, urljoin(base_url, "certs.html"), logger, config
    )
    if not res:
        return []
    ...
    selector = (
        ...
        .get("allfreedumps", {})
        .get("exam_link", "a[href$='-dumps.html']")
    )
...
async def worker_allfreedumps(
    ...
    async with semaphore:
        ...
        selector = (
            config.get("selectors", {})
            .get("allfreedumps", {})
            .get("pdf_download_link", "div.downbtn a")
        )
