...
async def discover_itexams_tasks(client: httpx.AsyncClient, config: Dict) -> List[Dict]:
    ...
    logger.info(
        "🔍 Discovering tasks from itexams.com via high-fidelity metadata crawl..."
    )
    tasks = []
    ...
    try:
        with open("discovered_vendors.json", "w") as f:
            json.dump(
                [
                    {"name": n, "url": urljoin(master_url, h)}
                    for n, h in zip(vendor_names, vendor_links)
                ],
                f,
                indent=2,
            )
    ...
    vendor_page_responses = await asyncio.gather(
        *[
            fetch_with_retry(client, urljoin(master_url, v_link), logger, config)
            for v_link in vendor_links
        ]
    )

    info_links_to_fetch = []
    for vendor_res in vendor_page_responses:
        logger.info(f"====== vendor_res {vendor_res}")
        if vendor_res:
    ...
    logger.info(
        f"🔍 Found {len(info_links_to_fetch)} '/info/' pages across all vendors."
    )

    # Step 4: Fetch all /info/ pages in parallel
    info_page_responses = await asyncio.gather(
        *[
            fetch_with_retry(client, info_link, logger, config)
            for info_link in info_links_to_fetch
        ]
    )
    ...
    for info_res in info_page_responses:
        ...
        vendor = normalize_vendor(
            exam_data.get("vendor", ""), exam_data.get("exam_code", "")
        )
    ...
    async with semaphore:
        try:
            ...
            if not exam_link_tag:
                logger.warning(
                    f"No exam page link found on info page: {task['info_url']}"
                )
                return []
            ...
            while current_url and page_count < 50:  # Safety break
                ...
                for q_container in q_containers:
                    ...
                    correct_elem = q_container.select_one(
                        selector.get("correct_answer")
                    )
                    if correct_elem:
                        correct_options = {
                            opt.strip() for opt in correct_elem.text.split(",")
                        }
                    ...
                    all_questions.append(
                        Question(
                            ...
                            tags=[task["vendor"], task["exam_code"]],
                ...
                current_url = (
                    urljoin(current_url, next_page_tag["href"])
                    if next_page_tag
                    else None
                )
