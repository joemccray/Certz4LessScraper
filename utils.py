...
def smart_hash(text: str, answers: List[Dict], explanation: str = "", version: str = "latest") -> str:
    """Generate a robust hash for a question to prevent duplicates."""
    sorted_answers = sorted([a['text'].strip() for a in answers])
    canonical_string = f"{version}|{text.strip()}|{'|'.join(sorted_answers)}|{explanation.strip()}"
...
@retry(stop=stop_after_attempt(5), wait=wait_exponential(multiplier=2, min=10, max=50), retry_error_callback=lambda retry_state: None)
...
def parse_pdf_task(pdf_content: bytes, metadata: Dict, config: Dict) -> List[Dict]:
    ...
    try:
        with fitz.open(stream=pdf_content, filetype="pdf") as doc:
            ...
            if len(native_text) < config['min_text_length_for_native_pdf'] or "NEW QUESTION" not in native_text.upper():
                if not OCR_ENABLED:
                    return []
        ...
        logger.error(f"Failed to read PDF content for {metadata.get('source_url')}: {e}")
        return []
    ...
    questions = []
    question_blocks = re.findall(r'(NEW QUESTION \d+[\s\S]+?)(?=NEW QUESTION \d+|$)', text, re.IGNORECASE)
    ...
    for block in question_blocks:
        ...
        correct_options = {opt.strip().upper() for opt in ans_match.group(1).split(',')} if ans_match else set()
        
        expl_match = re.search(r'Explanation:\s*([\s\S]+)', block, re.IGNORECASE)
        explanation = expl_match.group(1).strip() if expl_match else ""

        answers = [{"option": opt, "text": txt.strip(), "is_correct": (opt in correct_options)} for opt, txt in choices]
        if not answers: continue

        qid = smart_hash(question_text, answers, explanation, metadata.get("version", "latest"))
        
        questions.append(Question(
            question_id=qid, question_text=question_text, answers=answers, explanation=explanation,
            vendor=metadata.get("vendor"), exam_code=metadata.get("exam_code"), exam_name=metadata.get("exam_name"),
            tags=[metadata.get("vendor", "Unknown"), metadata.get("exam_code")], 
            source_url=metadata.get("source_url"), source_type=source_type
        ).dict())
    return questions
