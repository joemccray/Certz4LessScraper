...
async def validate_and_upsert_batch(records: List[Dict]):
    ...
    try:
        ...
        if response.data:
            main_logger.info(
                f"✅ Supabase upsert confirmed with {len(response.data)} entries."
            )
        else:
            main_logger.warning(
                "⚠️ Supabase upsert returned no data, check schema or payload."
            )
        return len(valid_records)
    except APIError as e:
        main_logger.error(
            f"❌ Supabase API error during upsert: {e.message}. Retrying..."
        )
        raise
    except Exception as e:
...
async def run_preflight_checks() -> bool:
    ...
    try:
        ...
        main_logger.info(
            "✅ Supabase connection successful and 'questions' table found."
        )
    except APIError as e:
        if 'relation "public.questions" does not exist' in e.message:
            main_logger.warning(
                "⚠️ Table 'questions' not found. Attempting to create it via RPC..."
            )
            try:
                await asyncio.to_thread(
            ...
            main_logger.critical(f"❌ Supabase API error: {e.message}")
            return False

    if shutil.which("tesseract") is None:
        main_logger.warning(
            "⚠️ Tesseract OCR engine not found in PATH. OCR fallback will fail."
        )
