...
def normalize_vendor(text_to_search: str, exam_code: str = "") -> str:
    ...
    for alias, canonical_name in VENDOR_ALIASES.items():
        logger.info(f"====== alias {alias} and canonical_name {canonical_name} and ====== lower_text {lower_text}")
        logger.info(f"{alias in lower_text}")
    ...
    for vendor in CANONICAL_VENDORS:
        logger.info(f"====== vendor {vendor} and lower_text {lower_text} and ====== vendor.lower() {vendor.lower()}")
        logger.info(f"{vendor.lower() in lower_text}")
